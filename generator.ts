...
/**
 * A Uint8ArrayReader that generates bytes based on provided custom byte data.
 * If no custom byte data is provided for a field, it generates zero bytes.
 */
export class Uint8ArrayGeneratorReader implements Uint8ArrayReader {
    constructor(private customByteData?: CustomBytes) {}

    /**
     * Reads `count` bytes for the field specified by `fieldPath`.
     * @param count The number of bytes to read.
     * @param fieldPath The path to the field in the structure (e.g., ["SectionName", "FieldName"]).
     * @returns A Uint8Array of the requested bytes.
     */
    ...
    /**
     * Peeks `count` bytes for the field specified by `fieldPath` without advancing the read position.
     * @param count  The number of bytes to peek.
     * @param fieldPath  The path to the field in the structure (e.g., ["SectionName", "FieldName"]).
     * @returns  A Uint8Array of the peeked bytes.
     */
...
/**
 * Generates a parsed structure based on the provided structure definition and optional custom byte data.
 * @param structDefinition The struct definition to generate the structure from.
 * @param customByteData Optional custom byte data to use for specific fields.
 * @returns  The generated parsed structure.
 */
...
/**
 * Generate a Uint8Array of bytes based on the provided structure definition and optional custom byte data.
 * @param structDefinition  The struct definition to generate bytes for.
 * @param customByteData  Optional custom byte data to use for specific fields.
 * @returns  A Uint8Array of the generated bytes.
 */
...
/**
 * Convert a parsed structure back into a Uint8Array of bytes.
 * @param struct The parsed structure to convert to bytes.
 * @returns A Uint8Array of the structure's bytes.
 */
...
/**
 * Generate a markdown table representing the fields of a section or field definition,
 * including nested fields, bitmasks, and enum mappings.
 * @param definition The field or section definition to generate the table for.
 * @param sectionDefinition  The parent section definition, used for context in nested fields.
 * @param level  The current heading level for nested fields (default is 1).
 * @returns  A string containing the markdown table and field descriptions.
 */
...
/**
 * Generate detailed descriptions for fields within a section or field definition,
 * including links to field names, enum mappings, and nested structures.
 * @param definition The field or section definition to generate descriptions for.
 * @param sectionDefinition The parent section definition, used for context in nested fields.
 * @param level The current heading level for nested fields (default is 1).
 * @returns A string containing the field descriptions in markdown format.
 */
...
/**
 * Genereate a markdown table for an enum mapping.
 * @param enumParser  The enum parser containing the mapping to generate the table for.
 * @returns A string containing the markdown table of the enum mapping.
 */
